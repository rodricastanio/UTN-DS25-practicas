...
function Secciones() {
  ...
  //llamada a la api de generos
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_URL}/api/generos`) // <-- unificar con tu back
      .then(res => res.json())
      .then(data => {
  ...
  const librosFiltrados = books.filter(libro => {  // <-- acá
  ...
  useEffect(
    () => {
      fetch(`${import.meta.env.VITE_API_URL}/api/books`)
        .then(res => res.json())
        .then(d => setBooks(Array.isArray(d) ? d : (d.books ?? d.items ?? [])))
    }, []
  )
      ...
      <div className="busqueda mt-8">
        ...
        {busqueda.trim() !== "" && (
          <>
            ...
            ">Mostrando {librosFiltrados.length} de {books.length} libros</p>
        ...
        gen.map((genero) => {
          ...
          return (
            ...
            >
              ...
              <TarjetaLibro genero={nombreGenero} libros={books} />  {/* acá también */}
            </section>
          );
        })
      }
